...
class Attachment:
    ...
    def __init__(
        ...
        Args:
            ...
            error: Whether there was an error parsing the attachment.
        """
        self.body = body
        self.source = source
        self.mime = mime
    ...
    def parse_body(
        ...
        """
        if body in (None, ''):
            return cls(body="Attachment body is None or empty", mime=Mime.TEXT, error=True)
    ...
    def parse_source(
        ...
        """
        if source in (None, ''):
            return cls(body="Attachment source is None or empty", mime=Mime.TEXT, error=True)
        ...
        if Mime.is_unsupported(mime):
            ...
            if inner_html == '':
                return Attachment(body=error_msg, mime=Mime.TEXT, error=True)
            else:
...
def _attachment_json(text: str | dict, indent: int = 4) -> Attachment:
    ...
    if not isinstance(text, (str, dict)):
        ...
        return Attachment(body=msg, mime=Mime.TEXT, error=True)
    try:
...
def _attachment_uri_list(text: str | list[str]) -> Attachment:
    ...
    try:
        ...
        return Attachment(body="Error parsing uri list.", mime=Mime.TEXT, error=True)
...
def _attachment_image(data: bytes | str, mime: str) -> Attachment:
    ...
    if isinstance(data, str):
        try:
            ...
            return Attachment(body="Error parsing image bytes.", mime=Mime.TEXT, error=True)
...
def _attachment_video(data: bytes | str, mime: str) -> Attachment:
    ...
    if isinstance(data, str):
        try:
            ...
            return Attachment(body="Error parsing video bytes.", mime=Mime.TEXT, error=True)
    return Attachment(body=data, mime=mime)
    ...
    if report.fx_html:
        if report.fx_single_page:
            ...
            if inner_html == '':
                ...
                mime = Mime.TEXT
                error = True
    return Attachment(body=text, mime=mime, inner_html=inner_html, error=error)
...
def _attachment_unsupported(text: str, mime, report):
    ...
    if report.fx_html:
        ...
        if inner_html == '':
            return Attachment(body="Error creating attachment from body", mime=Mime.TEXT, error=True)
